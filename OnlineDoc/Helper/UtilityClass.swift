...
@objc class UtilityClass: UIViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate,NVActivityIndicatorViewable {
...
//        }
    ...
    class func isInternetConnectedWith() -> Bool {
        
        let reachability: Reachability = try! Reachability.init(hostname: API_PREFIX)
        
        
       // let networkStatus: NetworkStatus = reachability.currentReachabilityStatus()
        
      //  if reachability.connection == .unavailable{//!reachability.isReachable && isAlert
...
//            activityIndicatorView = NVActivityIndicatorView(frame: CGRect(x: 0, y: 0, width: 50, height: 50), type: .ballRotateChase, color: .white, padding: 50)
...
//        RDGlobalFunction.appDelegate.window?.addSubview(activityView!)
        let activityData = ActivityData.init(size: CGSize.init(width: 50.0, height: 50.0), message: "Please wait...", type: .ballRotateChase)
