...
class RDGlobalFunction: NSObject {
    ...
    static var allDecks : [Deck] = []
    static var currentGame : Game!
    static var doesGameExist = false
    static var isNewJoin = false
    static var isStrangerGame = false
    static var isCurrentPlayerJudge = false
    ...
    //Store any data in UserDefault
    class func setDataInUserDefault(iSDefaultValue : Any, iSDefaultKey: String) {
        let encodedData = NSKeyedArchiver.archivedData(withRootObject: iSDefaultValue)
        UserDefaults.standard.set(encodedData, forKey: iSDefaultKey)
        UserDefaults.standard.synchronize()
    }
    
    class func getDataFromUserDefault(iSDefaultKey: String) -> Any {
        // if RDGlobalFunction.getBoolValueFromUserDefault(iSDefaultKey: RDDataEngineClass.iSUerActivated)  {
            let decoded  = UserDefaults.standard.object(forKey: iSDefaultKey) as! Data
    ...
    class func isValidUserName(userName:String) -> Bool{
        return userName.trimmingCharacters(in: .whitespacesAndNewlines).count > RDDataEngineClass.minUsernameLength && userName.trimmingCharacters(in: .whitespacesAndNewlines).count < RDDataEngineClass.maxUsernameLength
    }
    
    class func isValidPassword(password:String) -> Bool{
        return password.trimmingCharacters(in: .whitespacesAndNewlines).count > RDDataEngineClass.minPasswordLength
    }
...
//        return result
    ...
    class func showPickerSelection (delegate : UIViewController, type : selectionType, datasource : [Any]) {
        let vc = self.universalStory.instantiateViewController(withIdentifier: "SelectionPickerVC") as! SelectionPickerVC
        vc.modalPresentationStyle = .overCurrentContext
        vc.delegate = delegate as? selectionDelegate
        vc.currentType = type
   ...
   class func randomString(length: Int) -> String {
    ...
    class func getImageFromFirstCharacter(userName:String) -> UIImage {
        let firstLetter = userName.prefix(1).uppercased()
        return UIImage.init(named: firstLetter)!
    }
    
    
    //Relpace blank to Null
    class func removeNullFromDict (dict : [String:AnyObject]) -> [String:AnyObject] {
...
//    }
    ...
    class func setColorsToInput(input:MDCTextInputControllerOutlined) {
        input.activeColor = .white
        input.normalColor = .white
        input.inlinePlaceholderColor = .white
        input.floatingPlaceholderActiveColor = .white
        input.floatingPlaceholderNormalColor = .white
        if UIDevice.current.userInterfaceIdiom == .pad {
            input.textInputFont = UIFont.systemFont(ofSize: 22.0)
            input.inlinePlaceholderFont = UIFont.systemFont(ofSize: 22.0)
        } else {
            input.textInputFont = UIFont.systemFont(ofSize: 17.0)
            input.inlinePlaceholderFont = UIFont.systemFont(ofSize: 17.0)
        }
    }
    
    class func setGrayColorsToInput(input:MDCTextInputControllerOutlined) {
        input.activeColor = .lightGray
        input.normalColor = .lightGray
        input.inlinePlaceholderColor = .lightGray
        input.floatingPlaceholderActiveColor = .lightGray
        input.floatingPlaceholderNormalColor = .lightGray
        input.disabledColor = .lightGray
        if UIDevice.current.userInterfaceIdiom == .pad {
            input.textInputFont = UIFont.systemFont(ofSize: 22.0)
            input.inlinePlaceholderFont = UIFont.systemFont(ofSize: 22.0)
        } else {
    ...
    class func setColorsToMultilineInput(input:MDCTextInputControllerOutlinedTextArea) {
        input.activeColor = .lightGray
        input.normalColor = .lightGray
        input.inlinePlaceholderColor = .lightGray
        input.floatingPlaceholderActiveColor = .lightGray
...
extension UIView {
    ...
    func addShadowWithRoundCorners(radius:Float) {
